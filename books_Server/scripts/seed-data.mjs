...
async function seed() {
  ...
  // ── 2b. Link sub-categories to their parents (2nd pass) ────────────
  for (const cat of seedData.categories || []) {
    ...
    if (parentId && childId) {
      await categoriesCol.updateOne(
        { _id: childId },
        { $set: { parent: parentId, level: 1, updatedAt: now } }
      );
      console.log(`   ↳ Linked sub-category: ${cat.name} → ${cat.parentSlug}`);
    }
  }
  ...
  // ── 3b. Seed Site Content (branding singleton) ─────────────────────
  if (seedData.siteContent) {
    console.log('\n🎨 Seeding Site Content (branding)...');
    const sc = seedData.siteContent;
    const set = {};
    // Flatten to dot-notation so we update branding fields WITHOUT clobbering
    // other saved settings (e.g. admin-uploaded hero slides).
    for (const group of ['general', 'contact', 'footer', 'seo', 'theme', 'floating']) {
      if (sc[group] && typeof sc[group] === 'object') {
        for (const k of Object.keys(sc[group])) set[`${group}.${k}`] = sc[group][k];
      }
    }
    if (sc.defaultTagline !== undefined) set.defaultTagline = sc.defaultTagline;
    if (Array.isArray(sc.ticker)) set.ticker = sc.ticker;
    if (Array.isArray(sc.collectionBanners)) set.collectionBanners = sc.collectionBanners;
    set.updatedAt = now;
    await db.collection('sitecontents').updateOne(
      { _key: 'main' },
      { $set: set, $setOnInsert: { _key: 'main', createdAt: now } },
      { upsert: true }
    );
    console.log('   ✅ Store branding set to:', sc.general?.storeName || 'Books River');
  }
