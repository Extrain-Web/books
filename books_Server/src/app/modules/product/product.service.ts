...
const ProductService = {
    ...
    async getAllProducts(query: Record<string, unknown>) {
        ...
        // Merchandising flags: ?isFeatured / ?isOnSale / ?isNewProduct / ?isBestSelling / ?isFlashSale
        // = true/false → the storefront sections filter by the admin toggles.
        const flagConds: Record<string, unknown> = {};
        (['isFeatured', 'isOnSale', 'isNewProduct', 'isBestSelling', 'isFlashSale'] as const).forEach((f) => {
            if (query[f] === 'true' || query[f] === true) flagConds[f] = true;
            else if (query[f] === 'false' || query[f] === false) flagConds[f] = false;
            delete query[f];
        });
    ...
    // ── Get product by slug (or _id fallback) ───────────────────────────
    async getProductBySlug(slug: string) {
        // Trim + tolerate leading/trailing whitespace in stored slugs (legacy data
        // sometimes had trailing spaces that the URL bar drops → mismatch → 404).
        const clean = String(slug || '').trim();
        const escaped = clean.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const isId = /^[0-9a-fA-F]{24}$/.test(clean);
        const product = await Product.findOne({
            $or: [
                { slug: new RegExp('^\\s*' + escaped + '\\s*$', 'i') },
                ...(isId ? [{ _id: clean }] : []),
            ],
            isDeleted: { $ne: true },
        }).populate('category', 'name slug');
