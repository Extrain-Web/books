...
export function streamFinancePdf(res: Response, report: FinanceReport): void {
    ...
    doc.fillColor('#fff').fontSize(20).font('Helvetica-Bold').text('Bichitra Point', left, 26);
...
export async function buildFinanceXlsx(report: FinanceReport): Promise<Buffer> {
    ...
    wb.creator = 'Bichitra Point';
    wb.created = new Date();

    const headerFill = { type: 'pattern' as const, pattern: 'solid' as const, fgColor: { argb: 'FF0072BC' } };
    ...
    sum.addRow(['Bichitra Point — Financial Report']).font = { bold: true, size: 14 };
