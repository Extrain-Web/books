...
const initSslcommerzSession = async (
    ...
    const form: Record<string, string> = {
        ...
        cus_city: addr.district || addr.city || 'Dhaka',
...
const getAvailableMethods = async () => {
    ...
    const site = await SiteContent.findOne({ _key: 'main' }).select('payment contact').lean();
    const pay: any = site?.payment || {};
    const contact: any = site?.contact || {};
    const fallbackNumber = contact?.phone || '01739498553';
    ...
    for (const id of WALLETS) {
        ...
        if (provider?.isConfigured()) {
            ...
            methods.push({
                id,
                label: provider?.label || (id === 'bkash' ? 'bKash' : id === 'nagad' ? 'Nagad' : 'Rocket'),
                mode: 'manual',
                live: false,
                number: wallet.number || fallbackNumber,
                accountType: wallet.accountType || 'Personal',
            });
        }
    }

    if (pay?.sslcommerz?.active !== false) {
        methods.push({
            id: 'sslcommerz',
