...
export default function AuthLayout({
    ...
    return (
        <div className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-slate-50 px-4 py-12">
            ...
            {/* ── Go to Home button ── */}
            <div className="w-full max-w-[420px] mb-4 flex justify-start sm:absolute sm:top-6 sm:left-6 sm:max-w-none sm:w-auto z-20">
                <Link
                    href="/"
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white/90 hover:bg-white border border-slate-200 text-slate-700 hover:text-[var(--color-primary)] text-xs font-bold shadow-xs hover:shadow-md transition-all active:scale-95 backdrop-blur-xs"
                >
                    <LuArrowLeft size={14} />
                    Go to Home
                </Link>
            ...
            <Link href="/" className="relative z-10 mb-7 inline-flex items-center group transition-transform">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                    src="/Bichitra-Point-Logo-Colored.png"
                    alt="Bichitra Point"
                    className="h-[52px] md:h-[68px] w-auto object-contain drop-shadow-xs"
                />
            </Link>
