...
import {
    LuChevronLeft, LuInfo, LuCheck, LuCopy, LuLock, LuTag, LuCreditCard, LuTruck, LuX, LuPlus, LuMinus
} from 'react-icons/lu';
...
const BRAND: Record<string, { name: string; color: string; short: string }> = {
    ...
    rocket: { name: 'Rocket (DBBL)', color: '#8C3494', short: 'R' },
    sslcommerz: { name: 'Cards & Banking', color: '#1F6FEB', short: 'Cards' },
    cod: { name: 'Cash on Delivery', color: '#16a34a', short: 'COD' },
};

const PAYMENT_LOGOS: Record<string, string> = {
    bkash: '/images/payment/bkash.png',
    nagad: '/images/payment/nagad.png',
    rocket: '/images/payment/rocket.png',
};

const PaymentBrandIcon = ({ id }: { id: string }) => {
    ...
    if (src) {
        return (
            <img
                src={src}
                alt={id}
                className={id === 'rocket' ? "w-full h-full object-cover rounded-lg" : "w-9 h-9 object-contain drop-shadow-xs transition-transform duration-200"}
            />
        );
    }
    if (id === 'sslcommerz') {
        return <LuCreditCard size={20} className="text-white" />;
    }
    return <LuTruck size={20} className="text-white" />;
};
...
const labelClass =
    "block text-[14px] font-medium text-gray-700 mb-1.5";
...
const CheckoutPage = () => {
    ...
    const items = React.useMemo(() => {
        try {
            const sel = JSON.parse(localStorage.getItem('bichitrapoint_selected_cart') || localStorage.getItem('skawsarsunnahmart_selected_cart') || 'null');
    ...
    const rawMethods: PayMethod[] = methodsRes?.data?.methods || [];
    // Ensure Cash on Delivery (cod) is always at the very top
    const methods: PayMethod[] = React.useMemo(() => {
        return [...rawMethods].sort((a, b) => {
    ...
    const [formData, setFormData] = useState({
        fullName: '', email: '', phone: '', address: '', district: '', city: '', area: '', postalCode: '',
    });
    ...
    // ─── Shipping quote (debounced on district/city; recomputes when subtotal changes) ──
    const [debouncedDistrict, setDebouncedDistrict] = useState('');
    useEffect(() => {
        const t = setTimeout(() => setDebouncedDistrict((formData.district || formData.city || '').trim()), 400);
        return () => clearTimeout(t);
    }, [formData.district, formData.city]);

    const { data: shippingQuote } = useGetShippingQuoteQuery(
        { city: debouncedDistrict || undefined, subtotal: totalPrice } as any,
        { skip: totalPrice <= 0 },
    );

    const { data: shipSettings } = useGetShippingSettingsQuery();
    const freeThreshold = (shipSettings?.freeShippingByThresholdEnabled !== false)
        ? (shipSettings?.freeShippingThreshold ?? 1499)
        : 0;
    ...
    const freeShipping = couponFreeShipping || (shippingQuote?.freeShipping ?? (freeThreshold > 0 && totalPrice >= freeThreshold));
    const shippingCost = freeShipping ? 0 : (shippingQuote?.shippingCost ?? (freeThreshold > 0 && totalPrice >= freeThreshold ? 0 : (shipSettings?.defaultOutsideDhakaRate ?? 120)));
    ...
    // Auto-select the default address (or the first saved address) on mount
    useEffect(() => {
    ...
    useEffect(() => {
        // Don't bounce to the cart once the order is being placed or placed — the cart is emptied
        // on success and we want the confirmation modal to stay on screen.
        if (items.length === 0 && !placedOrder && !isSubmittingOrderRef.current) {
            router.push('/cart');
        }
    }, [items, router, placedOrder]);

    // Keep selected payment valid if admin hides the current method (defaults to cod if available)
    useEffect(() => {
        if (methods.length && !methods.some(m => m.id === selectedPayment)) {
            const hasCod = methods.some(m => m.id === 'cod');
            setSelectedPayment(hasCod ? 'cod' : methods[0].id);
        }
    }, [availableIds]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const persistAddressIfNeeded = async () => {
            ...
            const alreadySaved = savedAddresses.some(a =>
                ...
                norm(a.district || a.city) === norm(formData.district) &&
        ...
        const orderPayload = {
            items: items.map(item => {
                ...
                const pid = typeof rawId === 'string' && rawId.includes('_') ? rawId.split('_')[0] : rawId;
                return {
                    product: pid,
        ...
        try {
            if (isAuthenticated) {
                ...
                try {
                    localStorage.removeItem('bichitrapoint_selected_cart');
                    localStorage.removeItem('skawsarsunnahmart_selected_cart');
                } catch { }
                clearAppliedCoupons();
                ...
                {!isAuthenticated && (
                    <div className="mb-5 bg-blue-50 border border-blue-100 rounded-lg px-4 py-3 flex items-start gap-3">
                        ...
                        <p className="text-sm text-blue-700 leading-relaxed">
                            No account needed. We&apos;ll create one automatically — your phone number will be your login ID.{' '}
                    ...
                    <div className="flex flex-col lg:grid lg:grid-cols-12 gap-6 items-start">
                        ...
                        <div className="lg:col-span-7 space-y-6 w-full">
                            ...
                            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                                ...
                                {/* Saved addresses — auto-filled from dashboard */}
                                    ...
                                    <div className="px-5 pt-4 space-y-2.5">
                                        {savedAddresses.map((a: any) => {
                                            ...
                                            return (
                                                <label
                                                    key={a._id}
                                                    className={`flex items-start gap-3 px-4 py-3 rounded-lg border cursor-pointer transition-colors ${active ? 'bg-[var(--color-primary-lightest)]/40 border-[var(--color-primary)]' : 'border-gray-200 hover:border-gray-300'}`}
                                                >
                                                    ...
                                                    <span className="min-w-0">
                                                        ...
                                                        <span className="block text-xs text-gray-500 mt-0.5">{[a.address, a.area, a.district || a.city, a.postalCode].filter(Boolean).join(', ')}</span>
                                                    </span>
                                                </label>
                                            );
                                        })}
                                        <button
                                            type="button"
                                            onClick={() => { setSelectedAddressId('new'); setFormData(prev => ({ ...prev, address: '', district: '', city: '', area: '', postalCode: '' })); }}
                                            className={`w-full text-left px-4 py-3 rounded-lg border text-sm transition-colors ${selectedAddressId === 'new' ? 'bg-[var(--color-primary-lightest)]/40 border-[var(--color-primary)] text-gray-900' : 'border-dashed border-gray-300 text-gray-500 hover:border-gray-400'}`}
                                        >
                                ...
                                {(!isAuthenticated || savedAddresses.length === 0 || selectedAddressId === 'new') && (
                                    <div className="p-5 space-y-4">
                                        <div>
                                            ...
                                            <input type="text" name="fullName" value={formData.fullName} onChange={handleChange} placeholder="Full Name" className={cls('fullName')} />
                                            ...
                                            <div className="relative">
                                                <select
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        setFormData(prev => ({ ...prev, district: val, city: val }));
                                                        if (errors.district) setErrors(prev => { const n = { ...prev }; delete n.district; return n; });
                                                    }}
                                                    className={`${cls('district')} appearance-none cursor-pointer pr-10`}
                                                >
                                        ...
                                        {isAuthenticated && (
                                            <label className="flex items-center gap-2.5 pt-1 cursor-pointer select-none">
                                                ...
                                                />
                                                <span className="text-[14px] text-gray-700">Save this address to my account for faster checkout next time</span>
                                            </label>
                                        )}
                                    </div>
                                )}
                            ...
                            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                                ...
                                <div className="p-5 space-y-3">
                                        ...
                                        return (
                                            <label
                                                ...
                                                className={`group relative flex items-center gap-3.5 px-4 py-3.5 rounded-xl border cursor-pointer transition-all duration-200 ${
                                                    active
                                                        ? isCod
                                                            ? 'border-green-500 bg-green-50/20 shadow-sm ring-1 ring-green-500'
                                                            : 'shadow-[0_2px_12px_-4px_rgba(0,0,0,0.18)]'
                                                        : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50/60'
                                                }`}
                                                style={active && !isCod ? { borderColor: brand.color, background: `${brand.color}0A`, boxShadow: `0 0 0 1px ${brand.color}` } : {}}
                                            >
                                                <input
                                                    type="radio"
                                                    name="paymentMethod"
                                                ...
                                                {/* Brand tile */}
                                                <span
                                                    className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 shadow-xs overflow-hidden p-1"
                                                    style={{ background: isCod || method.id === 'sslcommerz' ? brand.color : '#ffffff', border: isCod || method.id === 'sslcommerz' ? 'none' : '1px solid #f1f5f9' }}
                                                >
                                                    <PaymentBrandIcon id={method.id} />
                                                </span>
                                                <span className="flex-1 min-w-0">
                                                    ...
                                                    <span className="block text-xs text-gray-500 mt-0.5 truncate">{SUBLINE[method.mode](method)}</span>
                                                </span>
                                    ...
                                    {selectedPayment === 'cod' && (
                                        <div className="rounded-xl border border-green-200 bg-green-50/70 p-4 flex items-start gap-3 mt-3">
                                            <div className="w-8 h-8 rounded-full bg-green-100 text-green-600 flex items-center justify-center flex-shrink-0 mt-0.5">
                                                <LuTruck size={16} />
                                            </div>
                                            <div>
                                                <p className="text-sm font-bold text-green-800">Pay when your order arrives!</p>
                                    ...
                                    {isGatewayMethod && activeMethod && (
                                        <div className="rounded-xl border px-4 py-4 mt-3"
                        ...
                        {/* ══ RIGHT COLUMN: Order Summary with Order Items ══ */}
                        <div className="lg:col-span-5 lg:sticky lg:top-24 h-fit w-full">
                            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                                <div className="px-5 py-4 border-b border-gray-100">
                                ...
                                <div className="px-5 py-2.5 bg-gray-50 border-b border-gray-100 flex items-center justify-between text-sm text-[14px] font-semibold text-gray-700">
                                    <span>Product</span>
                                    <span>Subtotal</span>
                                </div>

                                {/* Items List inside Order Summary */}
                                <div className="divide-y divide-gray-100 max-h-[380px] overflow-y-auto">
                                    {items.map((item) => (
                                        <div key={item.id} className="p-4 flex items-start gap-3 justify-between">
                                            <div className="flex items-start gap-2.5 min-w-0 flex-1">
                                                {/* Remove button */}
                                                <button
                                                    type="button"
                                                    onClick={() => dispatch(removeFromCart(item.id))}
                                                    className="text-gray-400 hover:text-red-500 p-0.5 mt-1 transition-colors flex-shrink-0"
                                                    title="Remove item"
                                                >
                                                    <LuX size={14} />
                                                </button>
                                                ...
                                                {/* Image */}
                                                <div className="w-13 h-13 bg-gray-50 rounded-lg border border-gray-100 p-1 flex-shrink-0 overflow-hidden flex items-center justify-center">
                                                ...
                                                <div className="min-w-0 flex-1">
                                                    ...
                                                    {/* Quantity Stepper */}
                                                    <div className="inline-flex items-center border border-gray-200 rounded mt-2 bg-white">
                                                        <button
                                                            type="button"
                                                            onClick={() => dispatch(decreaseQuantity(item.id))}
                                                            className="w-6 h-6 flex items-center justify-center text-gray-500 hover:text-gray-900 hover:bg-gray-50 text-xs transition-colors rounded-l"
                                                        >
                                                            <LuMinus size={11} />
                                                        </button>
                                                        ...
                                                        <button
                                                            ...
                                                            className="w-6 h-6 flex items-center justify-center text-gray-500 hover:text-gray-900 hover:bg-gray-50 text-xs transition-colors rounded-r"
                                                        >
                                                            <LuPlus size={11} />
                                                        </button>
                                                    </div>
                                    ...
                                    <div className="flex justify-between text-sm items-center">
                                        ...
                                        {freeShipping ? (
                                            <span className="font-semibold text-green-600">
                                                FREE {freeReasonLabel ? `(${freeReasonLabel})` : ''}
                                            </span>
                                        ) : (
                                            <span className="font-semibold text-gray-900">
                                                {(formData.district?.toLowerCase().includes('dhaka') ? 'Inside Dhaka: ' : 'Outside Dhaka: ')}
                                    ...
                                    <div className="flex justify-between items-baseline pt-3 border-t border-gray-100">
                                        ...
                                        <div className="text-right">
                                            {(couponDiscount > 0 || couponFreeShipping) && (
                                                <p className="text-xs line-through text-gray-400">৳{(totalPrice + (couponFreeShipping ? (shippingQuote?.shippingCost ?? (freeThreshold > 0 && totalPrice >= freeThreshold ? 0 : 120)) : shippingCost)).toLocaleString()}</p>
                                            )}
                                ...
                                <div className="p-5 pt-2 border-t border-gray-100">
                                    <button
                                        ...
                                        className="w-full flex items-center justify-center gap-2 py-3.5 bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] text-white rounded-lg text-sm font-bold transition-all shadow-md shadow-[var(--color-primary)]/20 disabled:opacity-60 disabled:cursor-not-allowed"
                                    >
                                        {isSubmitting ? (
                                            <>
