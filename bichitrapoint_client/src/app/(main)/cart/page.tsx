...
const SELECTED_STORAGE_KEY = 'bichitrapoint_selected_cart';
...
const CartItemVariants = ({
    ...
    const sizeList: string[] = useMemo(() => {
        if (!product) return [];
        if (hasVariants) return [...new Set(variants.filter((v: any) => v.size || v.label).map((v: any) => v.size || v.label))] as string[];
        return product.sizes?.length > 0 ? product.sizes : [];
    }, [product, hasVariants, variants]);

    const needsColor = colorSwatches.length > 0;
    const needsSize = sizeList.length > 0;
    // Variants are optional; if not selected/deselected, the product's base price and details apply.
    ...
    }, [product, item.id, onStatusChange]);

    // Auto-sync cart item price, mrp and thumbnail with the fresh product/variant data from backend
    useEffect(() => {
        if (!product) return;
        if (hasVariants && (item.color || item.size)) {
            const v = variants.find((vv: any) =>
                (!item.color || vv.color === item.color) &&
                (!item.size || vv.size === item.size || vv.label === item.size)
            );
            if (v && typeof v.price === 'number') {
                const correctPrice = v.price;
                const correctMrp = v.originalPrice || v.price;
                if (item.price !== correctPrice || item.mrp !== correctMrp) {
                    dispatch(updateCartItemVariant({
                        id: item.id,
                        color: item.color,
                        colorHex: item.colorHex || resolveHex(v.colorHex),
                        size: item.size,
                        price: correctPrice,
                        mrp: correctMrp,
                        image: item.image || v.images?.[0] || product.thumbnail,
                    }));
                }
            }
        } else if (typeof product.price === 'number') {
            const correctPrice = product.price;
            const correctMrp = product.originalPrice || product.price;
            if (item.price !== correctPrice || item.mrp !== correctMrp) {
                dispatch(updateCartItemVariant({
                    id: item.id,
                    price: correctPrice,
    ...
    }, [product, hasVariants, variants, item.id, item.color, item.size, item.price, item.mrp, dispatch]);
    ...
    const availableColors = useMemo(() => {
        ...
        return variants.filter((v: any) => v.size === item.size || v.label === item.size).map((v: any) => v.color).filter(Boolean);
    }, [hasVariants, item.size, variants, colorSwatches]);

    const availableSizes = useMemo(() => {
        if (!hasVariants || !item.color) return sizeList;
        return variants.filter((v: any) => v.color === item.color).map((v: any) => v.size || v.label).filter(Boolean);
    ...
    // If deselected, reverts to the base product's price, mrp and thumbnail!
    const apply = (color: string, size: string) => {
        let nextSize = size;
        if (hasVariants && color && nextSize) {
            const sizesForColor = variants.filter((v: any) => v.color === color).map((v: any) => v.size || v.label).filter(Boolean);
            if (sizesForColor.length > 0 && !sizesForColor.includes(nextSize)) nextSize = '';
        }

        let colorHex: string | undefined = color ? resolveHex(colorSwatches.find(c => c.name === color)?.hex || color) : undefined;
        let price: number | undefined;
        let mrp: number | undefined;
        let image: string | undefined;

        if (hasVariants) {
            const v = (color || nextSize)
                ? variants.find((vv: any) => (!color || vv.color === color) && (!nextSize || vv.size === nextSize || vv.label === nextSize))
                : null;
            if (v) {
                price = v.price;
                ...
                // Deselected / no variant selected → use base product values from API/DB
                price = product.price;
...
const CartPage = () => {
    ...
    const selectedSubtotal = selectedItems.reduce((s, i) => s + (Number(i.price) || 0) * (i.quantity || 1), 0);
    const selectedQty = selectedItems.reduce((s, i) => s + (i.quantity || 1), 0);
    ...
    // Live free-shipping threshold → "add ৳X more for free shipping" nudge (default 1499).
    ...
    const freeShipping = couponFreeShipping || (shippingQuote?.freeShipping ?? (freeThreshold > 0 && selectedSubtotal >= freeThreshold));
    const shippingCost = freeShipping ? 0 : (shippingQuote?.shippingCost ?? (freeThreshold > 0 && selectedSubtotal >= freeThreshold ? 0 : (shipSettings?.defaultOutsideDhakaRate ?? 120)));
    ...
    const originalShipping = couponFreeShipping
        ? (shippingQuote?.shippingCost ?? (freeThreshold > 0 && selectedSubtotal >= freeThreshold ? 0 : (shipSettings?.defaultOutsideDhakaRate ?? 120)))
        : shippingCost;
                                    ...
                                    <div key={item.id} className="group flex gap-3 sm:gap-4 px-4 sm:px-5 py-4">
                                        ...
                                        <div className="flex-1 min-w-0 flex flex-col">
                                            ...
                                            <div className="mt-1.5 flex items-baseline gap-2">
                                                <span className="text-sm font-semibold text-[var(--color-primary)]">৳{Number(item.price || 0).toLocaleString()}</span>
                                                {item.mrp && item.mrp > item.price && (
                                                    <>
                                                        <span className="text-xs text-gray-400 line-through">৳{Number(item.mrp).toLocaleString()}</span>
                                            ...
                                            <div className="mt-auto pt-3 flex items-end justify-between gap-2">
                                                ...
                                                <div className="flex items-end gap-2">
                                                    <div className="text-right">
                                                        <p className="text-[11px] text-gray-400">৳{Number(item.price || 0).toLocaleString()} × {item.quantity}</p>
                                                        <p className="text-base font-bold text-gray-900">৳{(Number(item.price || 0) * item.quantity).toLocaleString()}</p>
                                                    </div>
                                                    <button
                                                        onClick={() => setDeleteConfirmId(item.id)}
                    ...
                    <div className="lg:col-span-4 lg:sticky lg:top-24 h-fit">
                        <div className="bg-white rounded-lg shadow-sm">
                            ...
                            <div className="px-5 py-4 space-y-2.5">
                                ...
                                {freeThreshold > 0 && (
                                    <div className="bg-emerald-50 border border-emerald-200/80 rounded-lg p-2.5 text-emerald-800 space-y-1.5">
                                        <div className="flex items-center justify-between text-[11px] font-semibold">
                                            <span className="flex items-center gap-1.5">
                                                🚚 Free Shipping over <strong className="font-bold text-emerald-900">৳{freeThreshold.toLocaleString()}</strong>
                                            </span>
                                            {remainingForFree > 0 ? (
                                                <span className="font-bold text-emerald-700 bg-emerald-100/80 px-2 py-0.5 rounded text-[11px]">
                                                    Add ৳{remainingForFree.toLocaleString()} more
                                                </span>
                                            ) : (
                                        ...
                                        <div className="w-full bg-emerald-200/50 rounded-full h-1.5 overflow-hidden">
                                            <div
                                                ...
                                                style={{
                                                    width: `${Math.min(100, Math.max(5, Math.round((selectedSubtotal / freeThreshold) * 100)))}%`
                                                }}
                                            />
                                        </div>
                                        {remainingForFree > 0 ? (
                                            <p className="text-[10px] text-emerald-600">
                                                Shop <strong>৳{remainingForFree.toLocaleString()}</strong> more to reach <strong>৳{freeThreshold.toLocaleString()}</strong> for free delivery.
                                            </p>
                                        ) : (
