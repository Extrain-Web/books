...
const emptyAddress = {
    fullName: '', phone: '', address: '', district: '', city: '', area: '', postalCode: '', label: 'Home', isDefault: false,
};
...
export default function AddressesPage() {
    ...
    const openEdit = (addr: Address) => {
        ...
        const dist = addr.district || addr.city || '';
        setForm({
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {addresses.map((addr) => (
                        <div key={addr._id} className={`bg-white rounded-2xl border p-5 shadow-sm relative group transition-all ${
                            ...
                            <p className="text-sm text-gray-500">{addr.district || addr.city}{addr.area ? `, ${addr.area}` : ''}</p>
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                        ...
                        <form onSubmit={handleSubmit} className="p-5 space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 mb-2">District</label>
                                    <input type="text" required value={form.district || form.city} onChange={(e) => setForm({ ...form, district: e.target.value, city: e.target.value })}
