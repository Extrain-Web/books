...
import {
    ...
    LuX, LuMessageSquare, LuPackage, LuPhone, 
} from 'react-icons/lu';
import {
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-md border border-gray-200 shadow-sm">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">Manage customer contact messages and product inquiries</p>
                </div>
            ...
            <div className="grid grid-cols-3 gap-4">
                {[
                    { label: 'Total Messages', value: meta.total || inquiries.length, color: 'blue', icon: LuMessageSquare },
                ...
                <div className="flex-1 bg-white rounded-md border border-gray-200 shadow-sm overflow-hidden">
                    {isLoading ? (
                        ...
                        <div className="p-20 text-center">
                            ...
                            <h3 className="font-bold text-gray-700">No messages found</h3>
                            <p className="text-sm text-gray-400 mt-1">Customer contact messages and inquiries will appear here</p>
                        </div>
                    ) : (
                        <div>
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-[var(--color-primary)] font-medium truncate mt-0.5 flex items-center gap-1">
                                            {inq.product ? (
                                                <><LuPackage size={11} /> {inq.product?.name || inq.productName}</>
                                            ) : (
                                                <><LuMail size={11} /> {inq.subject || 'General Message'}</>
                                            )}
                                        </p>
                ...
                {selectedInquiry && (
                    <div className="w-[380px] flex-shrink-0 bg-white rounded-md border border-gray-200 shadow-sm overflow-hidden self-start sticky top-4">
                        ...
                        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                            <h3 className="font-bold text-gray-800">Message Detail</h3>
                        ...
                        <div className="p-5 space-y-5">
                            ...
                            <div className="space-y-3">
                                <h4 className="text-xs font-bold text-gray-400 uppercase">Sender</h4>
                                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-md">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <p className="font-bold text-gray-800 text-sm truncate">{selectedInquiry.name || selectedInquiry.customerName || 'Anonymous'}</p>
                                        {selectedInquiry.email && (
                                            <a href={`mailto:${selectedInquiry.email}`} className="text-xs text-gray-500 hover:text-[var(--color-primary)] flex items-center gap-1 truncate mt-0.5">
                                                <LuMail size={11} className="shrink-0" /> {selectedInquiry.email}
                                            </a>
                                        )}
                                        {selectedInquiry.phone && (
                                            <a href={`tel:${selectedInquiry.phone}`} className="text-xs text-gray-500 hover:text-[var(--color-primary)] flex items-center gap-1 truncate mt-0.5">
                                                <LuPhone size={11} className="shrink-0" /> {selectedInquiry.phone}
                                            </a>
                                        )}
                                    </div>
                                </div>
                            ...
                            {selectedInquiry.subject && (
                                <div className="space-y-1">
                                    ...
                                    <p className="text-sm font-semibold text-gray-800 bg-gray-50 p-2.5 rounded-md border border-gray-100">{selectedInquiry.subject}</p>
                                </div>
                            )}
