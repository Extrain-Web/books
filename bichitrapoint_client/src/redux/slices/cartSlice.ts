...
const loadCartFromStorage = (): CartState => {
    ...
    try {
        const stored = localStorage.getItem('bichitrapoint_cart') || localStorage.getItem('skawsarsunnahmart_cart');
        if (stored) {
...
const saveCartToStorage = (state: CartState) => {
    if (typeof window !== 'undefined') {
        try {
            localStorage.setItem('bichitrapoint_cart', JSON.stringify({
                items: state.items,
