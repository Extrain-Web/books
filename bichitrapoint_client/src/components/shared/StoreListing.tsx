...
const StoreListing: React.FC<StoreListingProps> = ({
    ...
    const isFeaturedParam = syncUrl ? (searchParams.get('isFeatured') || '') : '';
    const isNewProductParam = syncUrl ? (searchParams.get('isNewProduct') || '') : '';
    const isBestSellingParam = syncUrl ? (searchParams.get('isBestSelling') || '') : '';
    const isFlashSaleParam = syncUrl ? (searchParams.get('isFlashSale') || '') : '';
    const isOnSaleParam = syncUrl ? (searchParams.get('isOnSale') || '') : '';
    ...
    useEffect(() => {
        setPage(1);
    }, [lockedCategory, selectedCategory, activeSearch, priceRange.min, priceRange.max, sortBy, selectedBrand, minRating, inStockOnly, isFeaturedParam, isNewProductParam, isBestSellingParam, isFlashSaleParam, isOnSaleParam]);
    ...
    const queryParams = useMemo(() => {
        ...
        if (isFeaturedParam) p.isFeatured = isFeaturedParam;
        if (isNewProductParam) p.isNewProduct = isNewProductParam;
        if (isBestSellingParam) p.isBestSelling = isBestSellingParam;
        if (isFlashSaleParam) p.isFlashSale = isFlashSaleParam;
        if (isOnSaleParam) p.isOnSale = isOnSaleParam;
        return p;
    }, [page, sortBy, effectiveCategory, activeSearch, priceRange, selectedBrand, minRating, inStockOnly, isFeaturedParam, isNewProductParam, isBestSellingParam, isFlashSaleParam, isOnSaleParam]);
                ...
                <div className="flex gap-5">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        {isFetching && products.length === 0 ? (
                            ...
                            <div className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-2 sm:gap-2.5 transition-opacity duration-200 ${isFetching ? 'opacity-60' : 'opacity-100'}`}>
