...
import {
    ...
    LuSettings, LuDollarSign,
    LuGlobe, LuTrash2, LuDroplet, LuPercent, LuClock
} from 'react-icons/lu';
import {
...
const ProductFormInner = ({ productId: propProductId }: { productId?: string }) => {
    ...
    const [formData, setFormData] = useState<any>({
        ...
        price: '', originalPrice: '', wholesalePrice: '', discount: 0,
        // Offer window
        ...
        isFeatured: false, isNewProduct: true, isOnSale: false, isFlashSale: false, isBestSelling: false,
        // Visual variants
    ...
    useEffect(() => {
        if (isEditing && productToEdit?.data) {
            ...
            setFormData({
                ...formData,
                ...prod,
                isNewProduct: prod.isNewProduct !== undefined ? Boolean(prod.isNewProduct) : true,
                isFeatured: Boolean(prod.isFeatured),
                isOnSale: Boolean(prod.isOnSale),
                isBestSelling: Boolean(prod.isBestSelling),
    ...
    useEffect(() => {
        if (!isEditing && formData.name) {
            // URL-safe slug that keeps Unicode letters (Bengali) — spaces → dash, no
            // trailing spaces. Matches the backend cleaner so no whitespace slugs slip in.
            const slug = formData.name.trim().toLowerCase()
                .replace(/\s+/g, '-')
                .replace(/[^\p{L}\p{N}\p{M}-]+/gu, '') // keep combining marks (Bengali matras)
                .replace(/-+/g, '-')
                .replace(/^-+|-+$/g, '');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Remove legacy / omitted fields from payload
            delete payload.sku;
            delete payload.brand;
            delete payload.model;
            delete payload.tagline;
            delete payload.insideTheBox;
            delete payload.compatibility;
            delete payload.weight;
            delete payload.dimensions;
            delete payload.shippingConfig;
            delete payload.warranty;
            delete payload.codAvailable;
            delete payload.tags;
            delete payload.aiLabels;
            ...
            payload.wholesalePrice = toOptionalNum(formData.wholesalePrice);
            payload.stock = toOptionalNum(formData.stock) ?? 0;
            payload.lowStockThreshold = toOptionalNum(formData.lowStockThreshold) ?? 5;

            // Sanitize variants so that all numeric fields (including wholesalePrice) are clean numbers or null
            if (Array.isArray(payload.variants) && payload.variants.length > 0) {
                payload.variants = payload.variants.map((v: any) => ({
                    ...v,
                    price: toRequiredNum(v.price),
                    originalPrice: toOptionalNum(v.originalPrice) ?? null,
                    wholesalePrice: toOptionalNum(v.wholesalePrice) ?? null,
                    stock: toOptionalNum(v.stock) ?? 0,
                }));
            }
            ...
            // ── Merchandising flags: explicitly boolean values in database ──
            ...
            payload.isBestSelling = Boolean(formData.isBestSelling);
            payload.isFlashSale = Boolean(formData.isFlashSale);
            payload.status = formData.status || 'active';
            payload.visibility = formData.visibility || 'visible';
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div className="bg-white p-6 rounded-md border border-gray-200 shadow-sm space-y-5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                            <div className="space-y-1.5" data-field="price">
                                <label className="text-sm font-semibold text-gray-700">Retail Offer Price <span className="text-red-400">*</span></label>
                                <div className="relative">
                            ...
                            <div className="space-y-2" data-field="originalPrice">
                                <label className="text-sm font-semibold text-gray-700">Original / MRP Price</label>
                                <div className="relative">
                                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-sm">৳</span>
                            ...
                            <div className="space-y-2" data-field="wholesalePrice">
                                <label className="text-sm font-semibold text-emerald-800 flex items-center justify-between">
                                    <span>Wholesale Price</span>
                                    <span className="text-[10px] font-bold bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded">B2B / Optional</span>
                                </label>
                                <div className="relative">
                                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-emerald-600 font-bold text-sm">৳</span>
                                    <input type="number" name="wholesalePrice" placeholder="No wholesale price" className="w-full pl-8 pr-3 py-2.5 bg-emerald-50/40 border border-emerald-300 rounded-md outline-none text-base font-bold text-emerald-900 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-200" value={formData.wholesalePrice} onChange={handleChange} />
                                </div>
                                <p className="text-[11px] text-gray-400">Used when creating Wholesale Orders</p>
                            </div>
                            <div className="space-y-2">
                                <label className="text-sm font-semibold text-gray-700 flex items-center gap-1"><LuPercent size={14} /> Retail Discount</label>
                    ...
                    <div className="bg-white p-6 rounded-md border border-gray-200 shadow-sm space-y-5">
                        ...
                        <div className="flex gap-3 flex-wrap">
                            <button type="button" disabled={formData.colors.length === 0 && formData.sizes.length === 0} onClick={() => {
                                ...
                                const defWholesale = formData.wholesalePrice || null;
                                if (formData.colors.length > 0 && formData.sizes.length > 0) {
                                    formData.colors.forEach((c: string, ci: number) => {
                                        formData.sizes.forEach((s: string) => {
                                            ...
                                            newVars.push({ color: c, colorHex: formData.colorHex[ci] || '', size: s, description: formData.description || '', price: defPrice, originalPrice: defOrig, wholesalePrice: defWholesale, stock: 0, sku: '', images: [], note: '' });
                                        });
                                    });
                                } else if (formData.colors.length > 0) {
                                    formData.colors.forEach((c: string, ci: number) => {
                                        if (existing.some((v: any) => v.color === c && !v.size)) return;
                                        newVars.push({ color: c, colorHex: formData.colorHex[ci] || '', size: '', description: formData.description || '', price: defPrice, originalPrice: defOrig, wholesalePrice: defWholesale, stock: 0, sku: '', images: [], note: '' });
                                    });
                                } else if (formData.sizes.length > 0) {
                                    formData.sizes.forEach((s: string) => {
                                        if (existing.some((v: any) => v.size === s && !v.color)) return;
                                        newVars.push({ color: '', colorHex: '', size: s, description: formData.description || '', price: defPrice, originalPrice: defOrig, wholesalePrice: defWholesale, stock: 0, sku: '', images: [], note: '' });
                                    });
                                }
                            ...
                            <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 space-y-3">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                    <div className="space-y-1">
                                        ...
                                        <input type="number" placeholder="All retail price" className="w-full px-3 py-2 bg-white border border-gray-200 rounded-md text-sm outline-none focus:border-emerald-400" min="0"
                                    ...
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-bold text-emerald-700 uppercase">Wholesale (৳)</label>
                                        <input type="number" placeholder="All wholesale" className="w-full px-3 py-2 bg-emerald-50/50 border border-emerald-300 rounded-md text-sm outline-none focus:border-emerald-500 font-bold" min="0"
                                            onChange={e => { const v = Number(e.target.value); setFormData((prev: any) => ({ ...prev, variants: prev.variants.map((vr: any) => ({ ...vr, wholesalePrice: v > 0 ? v : null })) })); }} />
                                    </div>
                                    <div className="space-y-1">
                        ...
                        {(formData.variants || []).map((v: any, i: number) => (
                            <details key={i} className="border border-gray-200 rounded-lg overflow-hidden bg-gray-50">
                                <summary className="flex items-center gap-3 px-4 py-3 cursor-pointer select-none hover:bg-gray-100 transition-all" style={{ listStyle: 'none' }}>
                                    ...
                                    <span className="text-xs text-gray-400">৳{v.price || 0} {v.wholesalePrice ? `• WS: ৳${v.wholesalePrice}` : ''} • Stock: {v.stock || 0}{(v.images?.length || 0) > 0 ? ` • 📷${v.images.length}` : ''}</span>
                                ...
                                <div className="p-4 border-t border-gray-200 space-y-3">
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                        ...
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-bold text-emerald-700 uppercase">Wholesale (৳)</label>
                                            <input type="number" value={v.wholesalePrice ?? ''} placeholder="None" min="0" onChange={e => { const val = e.target.value === '' ? null : Number(e.target.value); const vars = [...formData.variants]; vars[i] = { ...vars[i], wholesalePrice: val }; setFormData((prev: any) => ({ ...prev, variants: vars })); }} className="w-full px-3 py-2 bg-emerald-50/40 border border-emerald-300 rounded-md text-sm outline-none focus:border-emerald-500 font-bold text-emerald-800" />
                                        </div>
                                        <div className="space-y-1">
                ...
                <div className="lg:col-span-4 space-y-6">
                    ...
                    <div className={`bg-white p-6 rounded-md border shadow-sm space-y-4 ${(errors.thumbnail || errors.images) ? 'border-red-300 ring-1 ring-red-200' : 'border-gray-200'}`} data-field="thumbnail">
                        ...
                        <p className="text-[11px] text-gray-400 -mt-2">Thumbnail is required. Gallery images are optional.</p>
                        <SingleImageUploader
                    ...
                    <div className="bg-white p-6 rounded-md border border-gray-200 shadow-sm space-y-4">
                        ...
                        <div className="space-y-2">
                            <Toggle label="New Arrival" name="isNewProduct" checked={Boolean(formData.isNewProduct)} onChange={handleChange} color="bg-emerald-500" />
                            <Toggle
                                label="⚡ Flash Sale"
                                name="isFlashSale"
                                ...
                                onChange={(e: any) => {
                                    ...
                                    setFormData((prev: any) => {
                                        const next = { ...prev, isFlashSale: checked };
                                        if (checked && !next.offerEndDate) {
                                            const defEnd = new Date();
                                            defEnd.setDate(defEnd.getDate() + 3);
                                            defEnd.setHours(23, 59, 0, 0);
                                            const pad = (n: number) => String(n).padStart(2, '0');
                                            next.offerEndDate = `${defEnd.getFullYear()}-${pad(defEnd.getMonth() + 1)}-${pad(defEnd.getDate())}T${pad(defEnd.getHours())}:${pad(defEnd.getMinutes())}`;
                                        }
                                        return next;
                                    });
                                }}
                            ...
                            {formData.isFlashSale && (
                                <div className="p-3 bg-rose-50 border border-rose-200 rounded-md space-y-1.5">
                                    <label className="text-xs font-bold text-rose-800 flex items-center gap-1.5">
                                        <LuClock size={13} className="text-rose-600" /> Flash Sale শেষ হওয়ার সময় (End Time)
                                    </label>
                                    <input
                                        type="datetime-local"
                                        name="offerEndDate"
                                        value={formData.offerEndDate ? (() => {
                                            const dt = new Date(formData.offerEndDate);
                                            if (isNaN(dt.getTime())) return formData.offerEndDate;
                                            const pad = (n: number) => String(n).padStart(2, '0');
                                            return `${dt.getFullYear()}-${pad(dt.getMonth() + 1)}-${pad(dt.getDate())}T${pad(dt.getHours())}:${pad(dt.getMinutes())}`;
                                        })() : ''}
                                        onChange={handleChange}
                                        className="w-full px-3 py-2 bg-white border border-rose-300 rounded-md text-xs font-semibold text-gray-800 outline-none focus:ring-1 focus:ring-rose-500"
                                    />
                                    <p className="text-[10.5px] text-rose-600 leading-tight">
                                        ⏰ এই সময় পার হলে প্রোডাক্টটি Flash Sale থেকে স্বয়ংক্রিয়ভাবে সরে যাবে এবং আসল দামে ফিরে যাবে।
                                    </p>
                                </div>
                            )}
                            <Toggle label="Featured Product" name="isFeatured" checked={Boolean(formData.isFeatured)} onChange={handleChange} color="bg-yellow-500" />
                            <Toggle label="On Sale" name="isOnSale" checked={Boolean(formData.isOnSale)} onChange={handleChange} color="bg-rose-500" />
                            <Toggle label="Top Selling" name="isBestSelling" checked={Boolean(formData.isBestSelling)} onChange={handleChange} color="bg-orange-500" />
                        </div>
                        <div className="pt-2">
