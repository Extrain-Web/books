...
const NotificationBell: React.FC<NotificationBellProps> = ({ theme = 'orange', seeAllHref }) => {
    ...
    const handleItemClick = async (n: NotificationItem) => {
        ...
        } else if (!target && n.meta?.orderId) {
            const isAdmin = theme === 'indigo' || seeAllHref.includes('/admin');
            target = isAdmin
                ? `/dashboard/admin/orders/${n.meta.orderId}`
                : `/dashboard/user/orders/${n.meta.orderId}`;
        }
