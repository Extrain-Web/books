                ...
                <div className="flex items-center gap-3">
                    {(category.icon || category.image) && (
                        <span className="shrink-0 w-11 h-11 rounded-lg bg-gray-50 border border-gray-100 flex items-center justify-center overflow-hidden text-2xl">
                            {category.image || (category.icon && (category.icon.startsWith('http') || category.icon.startsWith('/') || category.icon.startsWith('data:')))
                ...
                {subCategories.length > 0 && (
                    <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide mt-3 -mb-0.5 pb-0.5">
                        {subCategories.map((sub: any) => (
                            ...
                            >
                                {sub.icon && (
                                    sub.icon.startsWith('http') || sub.icon.startsWith('/') || sub.icon.startsWith('data:') ? (
                                        <img src={sub.icon} alt="" className="w-4 h-4 object-contain rounded shrink-0" />
                                    ) : (
                                        <span>{sub.icon}</span>
                                    )
                                )}
