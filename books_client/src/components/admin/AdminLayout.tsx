...
import { useGetAdminConversationsQuery } from '@/redux/api/chatApi';
import { useGetInquiriesQuery } from '@/redux/api/inquiryApi';
...
const menuSections: MenuSection[] = [
    ...
    // Wholesale operations
    {
        ...
        items: [
            {
                name: 'Wholesale', href: '/dashboard/admin/wholesale', icon: LuBuilding2, submenu: [
                    { name: 'Wholesale Orders', href: '/dashboard/admin/wholesale' },
                    { name: 'Create Wholesale Order', href: '/dashboard/admin/wholesale/new' },
                    { name: 'Wholesale Customers', href: '/dashboard/admin/wholesale/customers' },
                ]
            },
        ],
    },
    ...
    // What you sell
    {
        ...
        items: [
            ...
            {
                name: 'Categories', href: '/dashboard/admin/categories', icon: LuLayoutGrid, submenu: [
    ...
    // Configuration
    {
        ...
        items: [
            { name: 'Live Chat', href: '/dashboard/admin/chat', icon: LuMessageSquare, submenu: null },
            { name: 'Messages & Inquiries', href: '/dashboard/admin/inquiries', icon: LuMail, submenu: null },
...
const AdminLayout: React.FC<AdminLayoutProps> = ({ children }) => {
    ...
    // Live Unread Chat messages counter for Admin Sidebar
    const { data: chatData } = useGetAdminConversationsQuery({ status: 'all' }, { pollingInterval: 4000 });
    const unreadChatCount = chatData?.data?.stats?.unread ?? chatData?.meta?.stats?.unread ?? 0;

    // Pending Inquiries / Messages counter
    const { data: inqData } = useGetInquiriesQuery({ status: 'pending', limit: 1 }, { pollingInterval: 10000 });
    const pendingInquiriesCount = inqData?.meta?.total ?? inqData?.data?.filter((i: any) => i.status === 'pending')?.length ?? 0;
                                ...
                                <div key={item.name} style={{ marginBottom: '1px' }}>
                                    ...
                                    >
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                            {item.href === '/dashboard/admin/chat' && unreadChatCount > 0 && (
                                                <span style={{
                                                    background: '#ef4444',
                                                    color: '#ffffff',
                                                    fontSize: '10px',
                                            ...
                                            {item.href === '/dashboard/admin/inquiries' && pendingInquiriesCount > 0 && (
                                                <span style={{
                                                    background: '#f59e0b',
                                                    color: '#ffffff',
                                                    fontSize: '10px',
