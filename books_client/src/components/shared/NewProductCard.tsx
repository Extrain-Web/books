...
import { LuStar, LuX, LuSend, LuThumbsUp, LuCornerDownRight, LuHeart, LuShoppingCart } from 'react-icons/lu';
...
const NewProductCard: React.FC<NewProductCardProps> = ({ product }) => {
    ...
    const discountPercent = display.offerActive
        ? (Number(display.discount) > 0
            ? Number(display.discount)
    ...
    const handleAddToCart = (e: React.MouseEvent) => {
        ...
        e.stopPropagation();
        if (isInCart) {
            toast.success('Already added to cart', { id: `cart-${productId}` });
            return;
        }
        dispatch(addToCart({
            id: productId,
            ...
            mrp: oldPrice || product.originalPrice || product.mrp || product.price,
            image: product.image,
            category: product.categoryName || 'General',
        }));
        toast.success('Added to cart successfully!', { id: `cart-${productId}` });
    };
    // const soldCount = product.sold || product.soldCount || product.totalSold || 0;

    return (
        <>
            <div className='relative bg-white border border-gray-200 overflow-hidden hover:border-[var(--color-primary)]/40 hover:shadow-lg hover:shadow-slate-900/5 transition-all duration-300 group rounded-lg cursor-pointer select-none flex flex-col h-full'>
                <Link href={`/product/${product.slug || product.id}`} className='flex flex-col h-full cursor-pointer'>
                    <div className='flex flex-col h-full'>
                        {/* Product Image Area */}
                        <div className='relative w-full aspect-[4/5] bg-[#f8fafc] overflow-hidden flex items-center justify-center shrink-0'>
                            {/* Discount badge — People-Choice-style coral SALE tag */}
                            {discountPercent > 0 && (
                                <span className='absolute top-2 left-2 z-20 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm' style={{ background: 'var(--color-sale)' }}>
                                    -{discountPercent}%
                                </span>
                            )}
                            {/* Wishlist Button — hidden, slides in on card hover */}
                            <button
                                onClick={handleWishlistToggle}
                                className={`absolute top-2 right-2 z-20 w-8 h-8 rounded-full flex items-center justify-center shadow-md border transition-all duration-300 ease-out cursor-pointer hover:scale-110 active:scale-95 hover:shadow-lg bg-white/95 backdrop-blur-sm hover:bg-red-500 hover:border-red-500 hover:text-white ${isInWishlist ? 'text-red-500 opacity-100 translate-x-0 border-red-200' : 'text-slate-600 opacity-0 translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 border-slate-100'} ${wishlistAnim ? 'scale-125' : ''}`}
                        ...
                        {/* Product Info Panel — white background slides up together with title on hover */}
                        <div className='relative bg-white px-2.5 pt-2.5 pb-2.5 transition-transform duration-200 ease-out group-hover:-translate-y-[48px] z-10 -mb-[48px] flex-1 flex flex-col justify-between'>
                            <div>
                                {/* Product Name — strictly 2 lines fixed height */}
                                <h3 className='text-[13px] sm:text-[14px] text-slate-800 font-normal line-clamp-2 overflow-hidden leading-[18px] sm:leading-[20px] h-[34px] group-hover:text-[var(--color-primary)] transition-colors'>
                                    {product.name}
                                </h3>

                                {/* Price */}
                                <div className='flex items-baseline gap-1.5 mt-1.5'>
                                    <span className='text-[14px] sm:text-[15px] font-bold' style={{ color: 'var(--color-sale)' }}>৳{currentPrice.toLocaleString()}</span>
                                    {oldPrice && oldPrice > currentPrice && (
                                        <span className='text-[12px] sm:text-[14px] line-through text-slate-400'>৳{oldPrice.toLocaleString()}</span>
                                    )}
                                </div>
                            </div>

                            {/* Add to Cart Button with margin-top spacing */}
                            <div className='mt-3 pt-0.5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-out'>
                                <button
                                    onClick={handleAddToCart}
                                    className={`w-full py-2 px-3 rounded-md text-white text-[12px] font-semibold flex items-center justify-center gap-1.5 shadow-md active:scale-[0.98] cursor-pointer transition-all duration-200 ${isInCart
                                        ? 'bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] shadow-emerald-900/20'
                                        : 'bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] shadow-[var(--color-primary)]/25'
                                        }`}
                                    title={isInCart ? 'Already in Cart' : 'Add to Cart'}
                                >
                                    <LuShoppingCart size={13} className="shrink-0" />
                                    <span className="truncate">{isInCart ? 'Added to Cart' : 'Add to Cart'}</span>
                                </button>
                            </div>
                        </div>
                    </div>
...
const LIKED_REVIEWS_KEY = 'booksriver_liked_reviews';
const LIKED_REPLIES_KEY = 'booksriver_liked_replies';
...
const CommentsPopup: React.FC<{
    ...
    return (
        ...
        >
            <div
                className='bg-white rounded-lg w-full max-w-[680px] max-h-[92vh] flex flex-col overflow-hidden shadow-2xl'
            ...
            >
                ...
                <div className='shrink-0 border-b border-gray-200'>
                    <div className='w-full h-[360px] sm:h-[460px] bg-gray-50 flex items-center justify-center overflow-hidden p-2'>
                        <img
                            src={productImage}
                            alt={productName}
                            className='w-full h-full object-contain'
                            onError={(e) => {
                                (e.target as HTMLImageElement).src = 'https://via.placeholder.com/620x400/f3f4f6/9ca3af?text=No+Image';
                            }}
                        />
                    </div>
                    {/* Stats bar */}
