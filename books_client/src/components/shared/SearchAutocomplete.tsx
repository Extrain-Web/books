...
interface SearchAutocompleteProps {
    ...
    /** People-Choice style: 10px radius, thin border, no camera/voice, magnifier-only. */
    simple?: boolean;
}
...
const SearchAutocomplete: React.FC<SearchAutocompleteProps> = ({
    ...
    return (
        <div ref={rootRef} className="relative w-full min-w-0">
            <div
                className={`flex items-center w-full relative bg-white transition-shadow duration-200 focus-within:shadow-[0_0_0_3px_rgba(var(--color-primary-rgb),0.12)] ${
                    simple ? 'rounded-[10px]' : 'rounded-full'
                } ${isMobile ? 'h-[40px] sm:h-[44px]' : 'h-[48px]'}`}
                style={simple ? { border: '1px solid #d1d5db' } : { border: '2px solid var(--color-primary)' }}
            >
                {leading}

                {/* Leading search icon (hidden in simple/People-Choice mode) */}
                {!simple && (
                    <LuSearch
                        size={isMobile ? 15 : 18}
                        className={`shrink-0 text-gray-400 ${isMobile ? 'ml-3' : 'ml-3.5'}`}
                        strokeWidth={2}
                    />
                )}
                ...
                <input
                    ...
                    className={`flex-1 min-w-0 h-full text-gray-700 placeholder-gray-400 focus:outline-none text-xs sm:text-sm bg-transparent ${
                        simple ? 'pl-4 pr-2' : isMobile ? 'px-2' : 'px-3'
                    }`}
                    aria-label="Search"
                    autoComplete="off"
                />

                {/* Image / visual search — hidden in simple mode */}
                {!simple && (
                <button
                    type="button"
                    ...
                    className={`shrink-0 flex items-center justify-center rounded-full transition-colors hover:bg-[var(--color-primary-lightest)] ${
                        isMobile ? 'w-7 h-7 sm:w-8 sm:h-8' : 'w-9 h-9'
                    }`}
                    style={{ color: 'var(--color-primary)' }}
                >
                    <LuCamera size={isMobile ? 14 : 17} strokeWidth={2} />
                </button>
                )}

                {/* Voice search mic — hidden when unsupported or in simple mode */}
                {!simple && speechSupported && (
                    <button
                        type="button"
                        onClick={startVoice}
                        ...
                        className={`shrink-0 flex items-center justify-center rounded-full transition-colors ${
                            isMobile ? 'w-7 h-7 sm:w-8 sm:h-8 mr-1' : 'w-9 h-9 mr-1.5'
                    ...
                    >
                        <LuMic size={isMobile ? 14 : 17} strokeWidth={2} />
                    </button>
                )}
                ...
                {simple ? (
                    <button
                        ...
                        aria-label="Search"
                        className="shrink-0 flex items-center justify-center w-11 h-full text-slate-500 hover:text-[var(--color-primary)] transition-colors"
                    >
                        <LuSearch size={19} strokeWidth={2.2} />
                    </button>
                ) : (
                    <button
                        type="button"
                        onClick={() => submit(value)}
                        className={`shrink-0 rounded-full text-white font-semibold flex items-center justify-center gap-1.5 transition-opacity hover:opacity-90 active:opacity-75 ${
                            isMobile ? 'w-[32px] h-[32px] sm:w-auto sm:px-4 sm:h-[36px] mr-1 text-xs' : 'px-6 h-[40px] mr-1 text-sm'
                        }`}
                        style={{ background: 'var(--color-primary)' }}
                        aria-label="Search"
                    >
                        <LuSearch size={isMobile ? 14 : 16} strokeWidth={2.2} />
