    ...
    return (
        <section id="home-categories" className="w-full scroll-mt-24 bg-white border-b border-gray-100">
            <div className="container mx-auto px-4 sm:px-6 py-4 sm:py-5">
                ...
                {/* Header row — People-Choice-style two-tone heading + Explore pill */}
                <div className="flex items-center justify-between mb-4 sm:mb-5">
                    <h2 className="text-xl sm:text-2xl font-extrabold tracking-tight" style={{ color: '#2A1A12' }}>
                        Shop By <span style={{ color: 'var(--color-primary)' }}>Category</span>
                    </h2>
                    <div className="flex items-center gap-2">
                        <Link
                            href="/products"
                            className="flex items-center gap-1 text-xs sm:text-[13px] font-bold text-white px-3.5 sm:px-5 py-1.5 sm:py-2 rounded-full shadow-sm hover:brightness-110 active:scale-95 transition-all"
                            style={{ background: 'var(--color-primary)' }}
                        >
                            Explore <LuChevronRight size={15} />
                        </Link>
                        {onClose && (
                            <button
                                onClick={onClose}
                ...
                <div className="relative px-7 sm:px-0">
                    ...
                    >
                        <div className="flex gap-2 sm:gap-4 justify-start min-w-full py-2">
                            {categories.map(cat => (
                                <Link
                                    key={cat._id}
                                ...
                                >
                                    ...
                                    <div
                                        className="w-[76px] h-[76px] sm:w-[128px] sm:h-[128px] rounded-2xl bg-[var(--color-primary-surface)] border border-gray-100 overflow-hidden flex items-center justify-center transition-all duration-200 group-hover:border-[var(--color-primary-border)] group-hover:shadow-lg group-hover:-translate-y-0.5"
                                    >
                                        {cat.image || (cat.icon && (cat.icon.startsWith('http') || cat.icon.startsWith('/') || cat.icon.startsWith('data:'))) ? (
                                    ...
                                    {/* Label */}
                                    <span className="w-full text-[12px] sm:text-[14px] sm:font-semibold text-gray-600 text-center font-medium leading-snug transition-colors group-hover:text-[var(--color-primary)]">
                                        {cat.name}
                                    </span>
                                </Link>
                            ))}
