...
import {
    ...
    LuCircleCheck, LuInfo, LuShield, LuDroplet, LuTruck,
} from 'react-icons/lu';
import { useGetSiteContentQuery, useUpdateSiteContentMutation } from '@/redux/api/siteContentApi';
import { useGetShippingSettingsQuery, useUpdateShippingSettingsMutation } from '@/redux/api/shippingApi';
...
export default function SettingsPage() {
    ...
    const [updateContent, { isLoading: isSavingContent }] = useUpdateSiteContentMutation();
    const { data: shipSettings, isLoading: isShipLoading } = useGetShippingSettingsQuery();
    const [updateShipping, { isLoading: isSavingShip }] = useUpdateShippingSettingsMutation();

    const [formData, setFormData] = useState<any>(null);
    const [shipForm, setShipForm] = useState<any>(null);
        ...
        if (res?.data) {
            setFormData({
                general: res.data.general || {
                    storeName: 'Books River',
                    tagline: 'Your trusted online marketplace',
                    currency: 'BDT',
                },
                seo: res.data.seo || {
                    title: 'Books River - Your trusted online marketplace',
                    description: 'Shop the latest products with amazing deals at Books River.',
                    googleAnalyticsId: '',
                    facebookPixel: '',
                },
    ...
    useEffect(() => {
        if (shipSettings) {
            setShipForm({
                ...shipSettings,
                freeShippingThreshold: shipSettings.freeShippingThreshold ?? 1499,
            });
        }
    }, [shipSettings]);
    ...
    const isSaving = isSavingContent || isSavingShip;

    const handleSave = async () => {
        try {
            await updateContent(formData).unwrap();
            if (shipForm) {
                await updateShipping(shipForm).unwrap();
            }
            setSaveSuccess(true);
            ...
            <div className="space-y-6">
                ...
                <SettingSection icon={LuSettings} title="Store Information" description="Basic store identity">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <InputField label="Store Name" value={g.storeName || ''} onChange={(e: any) => updateGeneral('storeName', e.target.value)} placeholder="Books River" />
                        <InputField label="Tagline" value={g.tagline || ''} onChange={(e: any) => updateGeneral('tagline', e.target.value)} placeholder="Your trusted online marketplace" />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                ...
                <SettingSection
                    icon={LuTruck}
                    title="🚚 Delivery & Free Shipping Threshold"
                    description="Configure free shipping threshold (default ৳1499) and delivery rates shown across the website"
                >
                    {shipForm ? (
                        <div className="space-y-5">
                            <div className="p-4 rounded-xl border border-blue-100 bg-blue-50/50 space-y-4">
                                <label className="flex items-start gap-3 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        checked={Boolean(shipForm.freeShippingByThresholdEnabled)}
                                        onChange={(e) => setShipForm({ ...shipForm, freeShippingByThresholdEnabled: e.target.checked })}
                                        className="w-4 h-4 mt-0.5 accent-[var(--color-primary)] rounded"
                                    />
                                    <div>
                                        <span className="text-sm font-bold text-gray-800 block">
                                            Enable Free Shipping over Order Subtotal
                                        </span>
                                        <span className="text-xs text-gray-500">
                                            When enabled, customers whose order reaches this amount get free delivery automatically.
                                        </span>
                                    </div>
                                </label>
                                ...
                                <div className="max-w-md pt-2">
                                    <label className="block text-xs font-bold text-gray-700 uppercase mb-1.5">
                                        Free Shipping Minimum Amount (৳ BDT)
                                    </label>
                                    <div className="relative">
                                        ...
                                        <input
                                            type="number"
                                            min="0"
                                            className="w-full pl-8 pr-4 py-2.5 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--color-primary)] font-bold text-gray-900 outline-none text-base"
                                            value={shipForm.freeShippingThreshold ?? 1499}
                                            onChange={(e) => setShipForm({ ...shipForm, freeShippingThreshold: Number(e.target.value) })}
                                            placeholder="1499"
                                        />
                                    </div>
                                    <p className="text-[11px] text-gray-500 mt-1.5">
                                        Default: <strong>৳1499</strong>. Carts with ৳{(shipForm.freeShippingThreshold ?? 1499).toLocaleString()}+ will display FREE delivery.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-1">
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5">Inside Dhaka Standard Rate (৳)</label>
                                    <input
                                        type="number"
                                        min="0"
                                        className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[var(--color-primary)] outline-none font-medium text-sm"
                                        value={shipForm.defaultInsideDhakaRate ?? 60}
                                        onChange={(e) => setShipForm({ ...shipForm, defaultInsideDhakaRate: Number(e.target.value) })}
                                    />
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5">Outside Dhaka Standard Rate (৳)</label>
                                    <input
                                        type="number"
                                        min="0"
                                        className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[var(--color-primary)] outline-none font-medium text-sm"
                                        value={shipForm.defaultOutsideDhakaRate ?? 120}
                                        onChange={(e) => setShipForm({ ...shipForm, defaultOutsideDhakaRate: Number(e.target.value) })}
                                    />
                                </div>
                            </div>

                            <div className="flex items-center justify-between text-xs text-gray-500 pt-1 border-t border-gray-100">
                                <span>Advanced delivery zones and courier settings are managed in <a href="/dashboard/admin/shipping" className="text-[var(--color-primary)] font-bold underline">Shipping & Zones</a>.</span>
                            </div>
                        </div>
                    ) : (
                        <div className="py-4 text-center text-xs text-gray-400">Loading delivery settings...</div>
                    )}
                </SettingSection>

                {/* ═══════ SEO ═══════ */}
                <SettingSection icon={LuGlobe} title="SEO & Marketing" description="Search engine and tracking settings">
                    <InputField label="Meta Title" value={seo.title || ''} onChange={(e: any) => updateSeo('title', e.target.value)} placeholder="Books River - Your trusted online marketplace" helper="Recommended: 50-60 characters" />
