...
import {
    ...
    useUpdateAdminOrderItemsMutation,
...
export default function OrderDetailsPage() {
    ...
    const [updateOrderItems, { isLoading: isUpdatingItems }] = useUpdateAdminOrderItemsMutation();
    ...
    const openEditItemsModal = () => {
        ...
        setEditShippingCost(Number(order.shippingCost) || 0);
        setIsEditItemsModalOpen(true);
    };

    const handleItemQtyChange = (idx: number, newQty: number) => {
    ...
    const handleRemoveOrderItem = (idx: number) => {
        if (editItems.length <= 1) {
            toast.error('Order must have at least 1 item');
            return;
        }
    ...
    const handleAddProductFromModal = (item: {
        ...
        const existingIdx = editItems.findIndex(
            (i) => (i.product?._id || i.product) === item.product && (i.color || '') === (item.variantLabel || '')
        );
        ...
        if (existingIdx > -1) {
            ...
            setEditItems([
                ...editItems,
                {
                    ...
                    price: item.retailPrice || item.unitPrice,
                    quantity: item.quantity,
                    total: item.quantity * (item.retailPrice || item.unitPrice),
    ...
    const handleSaveOrderItems = async (e: React.FormEvent) => {
        e.preventDefault();
        ...
        try {
            ...
            toast.success('Order items and quantities updated');
            setIsEditItemsModalOpen(false);
        } catch (err: any) {
            toast.error(err?.data?.message || 'Failed to update order items');
        }
    };

    const editSubtotal = editItems.reduce((acc, curr) => acc + (curr.quantity * curr.price), 0);
    const editTotal = Math.max(0, editSubtotal - (editDiscount || 0) + (editShippingCost || 0));
    ...
    return (
        <div className="space-y-6 max-w-7xl mx-auto mb-10">
                    ...
                    <div className="bg-white rounded-md border border-gray-200 shadow-sm overflow-hidden">
                        <div className="p-5 border-b border-gray-100 flex items-center justify-between flex-wrap gap-2">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="bg-gray-100 text-gray-600 text-xs px-2 py-0.5 rounded-full font-bold">{order.items?.length || 0} Items</span>
                            </div>
                            <button
                                onClick={openEditItemsModal}
                                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-[var(--color-primary)] text-xs font-bold rounded-lg border border-blue-200 transition-all shadow-sm"
                            >
                                <LuPencil size={13} /> Edit / Add Products
                            </button>
                        </div>
            ...
            {/* Edit Order Items & Products Modal */}
            {isEditItemsModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden border border-gray-100 flex flex-col max-h-[90vh] animate-scaleUp">
                        {/* Modal Header */}
                        <div className="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-primary-dark)] text-white flex justify-between items-center">
                            <div>
                                <h3 className="font-bold text-base">Edit Order Items & Products</h3>
                                <p className="text-xs text-blue-100">Order: {order.orderId || order._id}</p>
                            </div>
                            <button
                        ...
                        {/* Modal Body */}
                        <form onSubmit={handleSaveOrderItems} className="p-6 overflow-y-auto space-y-6 text-xs flex-1">
                            {/* Products Header & Add Button */}
                            <div className="flex items-center justify-between border-b pb-3">
                                <div>
                                    <h4 className="font-bold text-sm text-gray-900">Order Products ({editItems.length})</h4>
                                    <p className="text-gray-500 text-[11px]">Add more products or adjust quantity and price</p>
                                </div>
                                <button
                                    type="button"
                                    onClick={() => setIsProductSelectOpen(true)}
                                    className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] text-white rounded-lg font-bold text-xs shadow-sm transition-all"
                                >
                            ...
                            <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="space-y-3">
                                    ...
                                    <div>
                                        <label className="block font-bold text-gray-700 mb-1">Shipping Cost (৳)</label>
                                        <input
                                            type="number"
                                            min="0"
                                ...
                                <div className="space-y-2 text-xs flex flex-col justify-center bg-white p-3.5 rounded-lg border border-gray-200">
                                    ...
                                    <div className="flex justify-between py-1 border-b border-gray-100 text-gray-600">
                                        <span>Shipping:</span>
                                        <span className="font-semibold">+ ৳ {editShippingCost.toLocaleString()}</span>
                                    </div>
                                    <div className="flex justify-between py-1.5 border-b-2 border-[var(--color-primary)] text-sm font-bold text-[var(--color-primary)]">
                                        <span>Total:</span>
                            ...
                            <div className="flex justify-end gap-3 pt-3 border-t">
                                ...
                                <button
                                    ...
                                    className="inline-flex items-center gap-1.5 px-5 py-2 bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] text-white rounded-lg font-bold shadow-md shadow-blue-600/20"
                                >
                                    <LuSave size={15} />
                                    {isUpdatingItems ? 'Saving Changes...' : 'Save & Update Order'}
                                </button>
                            </div>
                        </form>
                    </div>
