...
const TABS = [
    ...
    { key: 'collectionBanners', label: '🎞️ Banners', icon: LuImage },
...
export default function SiteContentPage() {
    ...
    return (
        <div>
            ...
            {activeTab === 'collectionBanners' && <CollectionBannersTab data={formData} setData={setFormData} />}
...
function ContactTab({ data, setData }: { data: any; setData: any }) {
    ...
    const loadDefaultSocials = () => {
        const defaults = [
            { label: 'Facebook', url: 'https://www.facebook.com/BooksRiver', color: '#3b5998' },
        ...
        <div>
            ...
            <div style={card}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                    ...
                    <div><label style={label}>Primary Email</label><input value={c.email || ''} onChange={e => updateField('email', e.target.value)} placeholder="support@booksriver.com" style={input} /></div>
                    <div><label style={label}>Official Website URL</label><input value={c.website || ''} onChange={e => updateField('website', e.target.value)} placeholder="https://booksriver.com" style={input} /></div>
...
function FooterTab({ data, setData }: { data: any; setData: any }) {
    ...
    return (
        <div style={card}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div><label style={label}>Company Name</label><input value={f.companyName || ''} onChange={e => update('companyName', e.target.value)} placeholder="Books River" style={input} /></div>
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                    ...
                    <textarea
                        ...
                        placeholder="Books River is a books and stationery retail chain, specializing in English medium textbooks, English novels, office stationery, and art & craft supplies. We deliver anywhere in Bangladesh within a maximum of 4 days."
                        rows={3}
                        style={{ ...input, resize: 'vertical' as const, fontFamily: 'inherit' }}
                    />
                </div>
...
/* ─── HERO SLIDES TAB ─── */
function CollectionBannersTab({ data, setData }: { data: any; setData: any }) {
    const banners = data.collectionBanners || [];
    const renumber = (arr: any[]) => arr.map((b: any, i: number) => ({ ...b, order: i }));
    const commit = (arr: any[]) => setData((p: any) => ({ ...p, collectionBanners: renumber(arr) }));

    const addBanners = (urls: string[]) => {
        const clean = (urls || []).filter(Boolean);
        if (!clean.length) return;
        commit([...banners, ...clean.map((imageUrl) => ({ imageUrl, active: true, link: '/products' }))]);
    };
    const update = (idx: number, field: string, value: any) =>
        commit(banners.map((b: any, i: number) => (i === idx ? { ...b, [field]: value } : b)));
    const remove = (idx: number) => commit(banners.filter((_: any, i: number) => i !== idx));
        ...
        <div>
            <div style={{ ...card, background: '#fffbeb', borderColor: '#fde68a' }}>
                <p style={{ fontSize: '12px', color: '#b45309', fontWeight: 600, margin: 0 }}>
                    🎞️ These banner cards show right after the &quot;Shop By Category&quot; section — usually two side-by-side (like O-Level / A-Level).
                    Upload an image per card; the <strong>title, description, button and link are all optional</strong>. Recommended size <strong>800 × 450</strong> (16:9).
                    Toggle <strong>Active</strong> to show/hide a card, reorder with ↑ ↓, then click <strong>Save Changes</strong>.
                </p>
            </div>

            {banners.length > 0 && (
                <div style={{ ...card }}>
                    <h3 style={{ fontSize: '14px', fontWeight: 700, margin: '0 0 12px' }}>Current Banners ({banners.length})</h3>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '14px' }}>
                        {banners.map((b: any, idx: number) => (
                            <div key={idx} style={{ borderRadius: '10px', overflow: 'hidden', border: '1px solid #e5e7eb', background: '#f9fafb' }}>
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                <img src={b.imageUrl} alt={`Banner ${idx + 1}`} style={{ width: '100%', height: '120px', objectFit: 'cover' }} />
                                <div style={{ padding: '8px', display: 'grid', gap: '6px' }}>
                                    <input value={b.title || ''} onChange={e => update(idx, 'title', e.target.value)} placeholder="Title (optional)" style={{ ...input, padding: '6px 8px', fontSize: '11px' }} />
                                    <input value={b.subtitle || ''} onChange={e => update(idx, 'subtitle', e.target.value)} placeholder="Description (optional)" style={{ ...input, padding: '6px 8px', fontSize: '11px' }} />
                                    <div style={{ display: 'flex', gap: '6px' }}>
                                        <input value={b.ctaLabel || ''} onChange={e => update(idx, 'ctaLabel', e.target.value)} placeholder="Button (optional)" style={{ ...input, padding: '6px 8px', fontSize: '11px' }} />
                                        <input value={b.link || ''} onChange={e => update(idx, 'link', e.target.value)} placeholder="/products" style={{ ...input, padding: '6px 8px', fontSize: '11px' }} />
                                    </div>
                                    <div style={{ display: 'flex', gap: '6px', alignItems: 'center', justifyContent: 'space-between' }}>
                                        <label style={{ fontSize: '11px', display: 'flex', gap: '4px', alignItems: 'center', cursor: 'pointer' }}>
                                            <input type="checkbox" checked={b.active !== false} onChange={e => update(idx, 'active', e.target.checked)} /> Active
                                        </label>
                                        <div style={{ display: 'flex', gap: '4px' }}>
                                            <button onClick={() => move(idx, 'up')} style={{ ...btnSmall, padding: '4px 9px' }} title="Move up">↑</button>
                                            <button onClick={() => move(idx, 'down')} style={{ ...btnSmall, padding: '4px 9px' }} title="Move down">↓</button>
                                            <button onClick={() => remove(idx)} style={{ ...btnDanger, padding: '4px 9px' }}>Remove</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div style={{ ...card }}>
                ...
                <p style={{ fontSize: '12px', color: '#666', margin: '0 0 10px' }}>
                    Select one or more images — each becomes a banner card. Add the title / description / button above after uploading (all optional).
                </p>
                <MultipleImageUploader label="" values={[]} onChange={(urls) => addBanners(urls)} max={6} />
            </div>
        </div>
    );
}
