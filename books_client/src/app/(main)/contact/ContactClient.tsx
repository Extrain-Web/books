...
export default function ContactClient() {
    ...
    const CONTACT_CARDS = [
        ...
        ...(c.warehouse ? [{
            ...
            label: 'Warehouse',
            primary: c.warehouse,
            secondary: 'Dispatch & Supply Hub',
            href: `https://maps.google.com/?q=${encodeURIComponent(c.warehouse)}`,
            accent: '#059669',
        }] : []),
    ];
    ...
    const TIPS = c.tips || [];
    const SOCIALS = (c.socials || []).filter((s: any) => s?.url && s.url !== '#');
            ...
            <div className="container" style={{ marginTop: '-1px' }}>
                ...
                <div style={{ display: 'flex', gap: '32px', flexWrap: 'wrap', marginBottom: '40px' }}>
                    ...
                    <div style={{ flex: '1 1 380px' }}>
                        ...
                        <form onSubmit={handleSubmit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                            ...
                            <button type="submit" disabled={isSubmitting} style={{
                                ...
                                padding: '12px 28px',
                                background: isSubmitting ? '#6b7280' : 'linear-gradient(135deg, var(--color-primary), #0d5c30)',
                                color: '#fff', border: 'none', borderRadius: '8px',
                                fontSize: '13.5px', fontWeight: 700, cursor: isSubmitting ? 'not-allowed' : 'pointer',
                                transition: 'all 0.2s ease', alignSelf: 'flex-start',
                                boxShadow: isSubmitting ? 'none' : '0 4px 14px rgba(79,70,229,0.35)',
                            }}
                                onMouseEnter={e => { if (!isSubmitting) { e.currentTarget.style.transform = 'translateY(-1px)'; e.currentTarget.style.boxShadow = '0 6px 20px rgba(79,70,229,0.45)'; } }}
                    ...
                    <div style={{ flex: '0 0 280px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                        ...
                        {/* Social Quick Links */}
                        {SOCIALS.length > 0 && (
                            <div style={{ border: '1.5px solid #e5e7eb', borderRadius: '12px', padding: '16px 18px', background: '#fff' }}>
                                ...
                                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                    {SOCIALS.map((s: any, i: number) => (
                                        <a key={i} href={s.url} target="_blank" rel="noopener noreferrer" style={{
                                            padding: '6px 12px',
                                            background: `${s.color || '#0072bc'}15`,
                                            color: s.color || '#0072bc',
                                            fontSize: '11.5px', fontWeight: 600,
                                            borderRadius: '6px', textDecoration: 'none',
                                            transition: 'all 0.2s ease',
                                            border: `1px solid ${s.color || '#0072bc'}30`,
                                        }}
                                            onMouseEnter={e => { e.currentTarget.style.background = s.color || '#0072bc'; e.currentTarget.style.color = '#fff'; }}
                                            onMouseLeave={e => { e.currentTarget.style.background = `${s.color || '#0072bc'}15`; e.currentTarget.style.color = s.color || '#0072bc'; }}
                                        >
                                            {s.label}
                                        </a>
                                    ))}
