...
productSchema.pre('save', function (next) {
    // ── Slug: always URL-safe (trim, spaces → dash, keep Unicode letters/digits) ──
    // A raw/Bengali slug with spaces or trailing whitespace breaks the
    // /products/slug/:slug lookup (the browser trims trailing %20 → 404). Clean any
    // provided slug so it never contains whitespace; generate from name if empty.
    // Keep Unicode letters, numbers AND combining marks (\p{M}) — Bengali matras /
    // hasant are marks; dropping them corrupts the word (টেস্ট → টসট).
    const cleanSlug = (str: string) => String(str || '').trim().toLowerCase()
        .replace(/\s+/g, '-')
        .replace(/[^\p{L}\p{N}\p{M}-]+/gu, '')
        .replace(/-+/g, '-')
        .replace(/^-+|-+$/g, '');
    if (this.slug) this.slug = cleanSlug(this.slug);
    if (!this.slug) this.slug = (cleanSlug(this.name) || 'product') + '-' + Date.now();
