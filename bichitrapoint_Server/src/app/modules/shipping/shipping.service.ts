...
export async function getSettings(): Promise<IShippingSettings> {
    let doc = await ShippingSettings.findOne({ _key: 'main' });
    if (!doc) {
        doc = await ShippingSettings.create({ _key: 'main', freeShippingThreshold: 1499 });
    } else if (doc.freeShippingThreshold === 3000 || doc.freeShippingThreshold === 5000) {
        doc.freeShippingThreshold = 1499;
        await doc.save();
    }
    return doc;
}
...
export async function computeShippingCost(
    { district, city, subtotal, items, totalQuantity, couponFreeShipping }: ShippingQuoteInput,
): Promise<ShippingQuoteResult> {
    const sub = Number(subtotal) || 0;
    const cityStr = (district || city || '').toString().trim().toLowerCase();
