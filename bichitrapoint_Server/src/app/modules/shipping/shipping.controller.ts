...
const ShippingController = {
    ...
    getQuote: catchAsync(async (req: Request, res: Response) => {
        ...
        const quote = await computeShippingCost({ district, city, subtotal });
    ...
    getShipments: catchAsync(async (req: Request, res: Response) => {
        ...
        const shipmentRecords = shipments.map((order: any) => ({
            ...
            destination: order.shippingAddress ? `${order.shippingAddress.district || order.shippingAddress.city || ''}, ${order.shippingAddress.area || ''}` : 'N/A',
