...
const buildVerificationEmail = (verifyLink: string): { subject: string; html: string } => ({
    subject: 'Verify your Bichitra Point email',
    html: `
        <div style="font-family: Arial, sans-serif; max-width: 480px; margin: auto;">
            <h2 style="color: #0072BC;">Welcome to Bichitra Point!</h2>
            <p>Please confirm your email address to activate your account.</p>
            <p style="text-align: center; margin: 28px 0;">
                <a href="${verifyLink}" style="background: #0072BC; color: #fff; padding: 12px 28px; border-radius: 6px; text-decoration: none; display: inline-block;">Verify Email</a>
            </p>
...
const buildOtpEmail = (otp: string, purpose: string): { subject: string; html: string } => ({
    subject: `Your Bichitra Point verification code: ${otp}`,
    html: `
        <div style="font-family: Arial, sans-serif; max-width: 480px; margin: auto;">
            <h2 style="color: #0072BC;">Your verification code</h2>
            <p>Use the code below to continue${purpose ? ` (${purpose})` : ''}:</p>
            <p style="text-align: center; font-size: 32px; letter-spacing: 8px; font-weight: bold; color: #0072BC; margin: 24px 0;">${otp}</p>
...
const buildResetEmail = (resetLink: string): { subject: string; html: string } => ({
    subject: 'Reset your Bichitra Point password',
    html: `
        <div style="font-family: Arial, sans-serif; max-width: 480px; margin: auto;">
            <h2 style="color: #0072BC;">Password reset request</h2>
            <p>We received a request to reset your password. Click the button below to choose a new one.</p>
            <p style="text-align: center; margin: 28px 0;">
                <a href="${resetLink}" style="background: #0072BC; color: #fff; padding: 12px 28px; border-radius: 6px; text-decoration: none; display: inline-block;">Reset Password</a>
            </p>
...
const AuthService = {
    ...
    async register(payload: any): Promise<IAuthResponse & { verifyLink?: string }> {
        ...
        const userEmail = email || `${phone?.replace(/\s+/g, '')}@guest.bichitrapoint.com`;
    ...
    async googleLogin(input: { code?: string; idToken?: string; accessToken?: string }): Promise<IAuthResponse> {
        ...
        if (code) {
            ...
            if (config.google.client_id && payload.aud !== config.google.client_id) {
                throw new AppError(401, 'This Google sign-in is not authorized for Bichitra Point.');
            }
        } else if (idToken) {
